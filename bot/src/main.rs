...
lazy_static! {
    ...
    static ref RELOAD_HANDLER: Arc<Mutex<DynamicReload<'static>>> = Arc::new(Mutex::new(
        DynamicReload::new(Some(vec!["plugins"]), Some("plugins"), Search::Backwards)
    ));
...
fn main() {
    ...
    let config = match Config::load("config.toml") {
        ...
        Err(e) => {
            error!(
                "Unable to load config.toml for IRC configuration with error {:?}, exiting!",
                e
            );
            std::process::exit(1);
        }
    ...
    let mut reactor = match IrcReactor::new() {
        ...
        Err(e) => {
            error!(
                "Unable to initialize IrcReactor with error {:?}, exiting!",
                e
            );
            std::process::exit(1);
        }
    ...
    let client = match reactor.prepare_client_and_connect(&config) {
        ...
        Err(e) => {
            error!(
                "Unable to initialize IrcClient with error {:?}, exiting!",
                e
            );
            std::process::exit(1);
        }
            ...
            if let Command::PRIVMSG(ref chan, ref msg) = message.command {
                ...
                if msg == "!reload" && config.is_owner(nick) {
                    ...
                    RELOAD_HANDLER
                        .lock()
                        .unwrap()
                        .update(Plugins::reload_callback, &mut PLUGINS.lock().unwrap());
                    PLUGINS.lock().unwrap().initialize_all(client);
                    client
                        .send_privmsg(&chan, "Reloaded plugins successfully.")
                        .unwrap_or_else(|e| {
                    ...
                    if let Some(name) = caps.get(1) {
                            ...
                            .add_library(&name, PlatformName::Yes)
                        {
                            Ok(lib) => {
                                ...
                                client
                                    .send_privmsg(&chan, &format!("Successfully loaded {}", name))
                                    .unwrap_or_else(|e| {
                                        warn!("send_privmsg failed for plugin load notice with error {:?}", e);
                                    });
                            }
                            Err(_) => {
                                client
                                    .send_privmsg(&chan, &format!("Unable to load {}", name))
                                    .unwrap_or_else(|e| {
                                        warn!("send_privmsg failed for plugin load failure notice with error {:?}", e);
                                    });
                            }
                        }
                    }
    ...
    ctrlc::set_handler(|| {
        ...
        error!(
            "Failed to register SIGINT/SIGTERM handler with error {:?}",
            e
        );
    });
